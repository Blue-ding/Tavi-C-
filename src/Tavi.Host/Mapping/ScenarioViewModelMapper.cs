...
{
    internal static ScenarioWorkspaceViewModel ToWorkspace(IScenarioWorkspace workspace, IReadOnlyList<SceneDefinition> definitions)
    {
        ScenarioSnapshot snapshot = workspace.Queries.CreateSnapshot();
        ...
        return new ScenarioWorkspaceViewModel(snapshot.Id, snapshot.SourceWorldStateId, workspace.IsDirty, workspace.CanUndo, workspace.CanRedo, workspace.Health.ToString(), modules, elements, aspects, relations, scopes, mappedDefinitions, scenes);
    }
