...
namespace Tavi.Domain.World
{
    ...
    public sealed class WorldGraph
    {
        ...
        public IReadOnlyCollection<Relation> GetRelations(Guid anchorId)
        {
            ...
            return EnumerateAllRelations()
                .Where(relation =>
                    relation.SourceId == anchorId || relation.TargetId == anchorId
                )
                .ToArray();
        }
        ...
        public Guid AddAnchor(string name, string description, AnchorType type)
        {
            ...
            EnsureDescription(description, operation, nameof(description));
        ...
        )
        {
            ...
            SubWorldData subWorld = FindSubWorld(domainId.Value)
                                    ?? CreateSubWorldData(domainId.Value);
        ...
        {
            RelationLocation location = FindRelation(
                relationId,
                nameof(RemoveRelation)
            );
        ...
        public void UpdateAnchorName(Guid anchorId, string name)
        {
            ...
            Anchor anchor = GetAnchorForOperation(anchorId, operation, "Anchor");
            if (
                anchor.Type == AnchorType.Character
                && _characterIdsByName.TryGetValue(name, out Guid duplicateId)
        ...
        public void UpdateAnchorType(Guid anchorId, AnchorType type)
        {
            ...
            bool becomesCharacter =
                anchor.Type != AnchorType.Character
                && type == AnchorType.Character;
            bool stopsBeingCharacter =
                anchor.Type == AnchorType.Character
                && type != AnchorType.Character;

            if (
                becomesCharacter
                && _characterIdsByName.TryGetValue(
                    anchor.Name,
        ...
        public void UpdateRelationDescription(
            Guid relationId,
            string description
        )
        {
            ...
            EnsureDescription(description, operation, nameof(description));
            FindRelation(relationId, operation).Relation.UpdateDescription(
                description
            );
        }
        ...
        private Anchor GetAnchorForOperation(
            Guid anchorId,
            string operation,
            string entityName
        )
        ...
        {
            Anchor anchor = GetAnchorForOperation(
                characterId,
                operation,
                "Domain Anchor"
            );
        ...
        {
            return _data.SubWorldData.Find(
                subWorld => subWorld.DomainId == characterId
            );
        }
        ...
        private RelationLocation FindRelation(
            Guid relationId,
            string operation
        )
        {
            ...
            {
                if (
                    subWorld.Relations.TryGetValue(
                        relationId,
                        out relation
                    )
                )
        ...
        private static void EnsureDescription(
            [NotNull] string? value,
            string operation,
            string parameterName
        )
        {
            if (value is null)
            {
                throw new WorldGraphException(
                    WorldGraphErrorCode.InvalidArgument,
                    operation,
                    $"参数 {parameterName} 不能为 null。"
                );
            }
        }

        private static void EnsureAnchorType(
            AnchorType type,
            string operation
        )
        {
        ...
        private static WorldGraphException NotFound(
            string operation,
            string entityName,
            Guid entityId
        )
        ...
        private static void ValidateWorldData(WorldData? data)
        {
            ...
            var characterNames = new Dictionary<string, Guid>(
                StringComparer.Ordinal
            );
        ...
        private static void ValidateAnchor(
            ...
            Anchor anchor,
            string path,
            Dictionary<string, Guid> characterNames,
            List<string> errors
        )
        {
            if (key == Guid.Empty)
            ...
            )
            {
                ...
                {
                    errors.Add(
                        $"{path}.Name“{anchor.Name}”与 Character {duplicateId} 重复。"
                    );
                }
                else
                {
        ...
        private static void ValidateRelations(
            Dictionary<Guid, Relation> relations,
            string path,
            Dictionary<Guid, Anchor>? anchors,
            HashSet<Guid> relationIds,
            List<string> errors
        )
        {
            foreach ((Guid key, Relation? relation) in relations)
            {
                ...
                {
                    errors.Add(
                        $"{relationPath} 的字典键与 Relation.Id {relation.Id} 不一致。"
                    );
                }
                if (
                ...
                {
                    errors.Add(
                        $"{relationPath}.Id {relation.Id} 在世界图中重复。"
                    );
                }
                if (string.IsNullOrWhiteSpace(relation.Name))
                {
                    errors.Add(
                        $"{relationPath}.Name 不能为空或只包含空白字符。"
                    );
                }
                if (relation.Description is null)
                    errors.Add($"{relationPath}.Description 不能为 null。");
                if (
                    anchors is null
                    || !anchors.ContainsKey(relation.SourceId)
                )
                {
                    errors.Add(
                        $"{relationPath}.SourceId {relation.SourceId} 不指向任何 Anchor。"
                    );
                }
                if (
                    anchors is null
                    || !anchors.ContainsKey(relation.TargetId)
                )
                {
                    errors.Add(
                        $"{relationPath}.TargetId {relation.TargetId} 不指向任何 Anchor。"
                    );
                }
            }
        }
