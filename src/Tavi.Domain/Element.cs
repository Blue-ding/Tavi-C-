...
/// <summary>表示可被一元或二元断言引用的领域实体；其类型不赋予任何 EARS 结构特权。</summary>
public sealed record Element
{
    ...
    /// <param name="type">由 Tavi 或 Module 定义的开放类型。</param>
    ...
    /// <summary>更新 Element 名称；调用方必须位于所属状态的事务边界内。</summary>
    internal void UpdateName(string name) => Name = name;

    /// <summary>更新 Element 说明；调用方必须位于所属状态的事务边界内。</summary>
    internal void UpdateDescription(string description) => Description = description;

    /// <summary>更新 Element 类型；调用方必须位于所属状态的事务边界内。</summary>
